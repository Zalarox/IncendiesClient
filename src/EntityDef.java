...
public final class EntityDef {
	...
	/**
	 * aByte68 = sizes
	 * Also, remember to hardcode size for fishing spots
	 * 
	 */
	...
	public static EntityDef forID(int i) {
		...
		case 2566:
			entityDef.actions = new String[5];
			entityDef.actions[0] = "Talk-to";
			entityDef.actions[2] = "Buy skill cape";
			break;
		}
		return entityDef;
	}
	...
	/*public static void writeEverything() {
		...
		if(i1 >= 1)
			return;
		try {
			writer = new BufferedWriter(new FileWriter("npcSizes.txt"));
		} catch (IOException e) {
		}
		for (int j = 0; j < 11599; j++) {
			forID(j);
		}
		for (int j = c; j < 11599; j++) {
			forID(j);
		}
		for (int j = c; j < 11599; j++) {
			forID(j);
		...
		System.out.println("finished dumping "+c);
		System.out.println("Size:"+ forID(2745).aByte68);
		System.out.println("Size:"+ forID(1).aByte68);
	}*/
	...
	public Model method164(int j, int k, int ai[]) {
		...
		if (model == null) {
			...
			model.method469();
			model.method479(frontLight, backLight, rightLight, middleLight, leftLight, true);
