...
public class Sprite extends DrawingArea {
	...
	public void drawTooltippedSprite(Client c, int xPos, int yPos, String[] toolTipText, boolean advancedSprite) {
		if(advancedSprite) 
			drawAdvancedSprite(xPos, yPos);
		else
			drawSprite(xPos, yPos);
		if(c.mouseX >= xPos - myWidth && c.mouseX <= xPos + myWidth && c.mouseY >= yPos - myHeight && c.mouseY <= yPos + myHeight)
			c.drawTooltip(c.mouseX + 11, c.mouseY - 20, toolTipText, false, 0);
	}
	
	public void drawRegisterSprite(Client c, int xPos, int yPos, String[] toolTipText, boolean advancedSprite, int info) {
		if(advancedSprite) 
			drawAdvancedSprite(xPos, yPos);
		else
			drawSprite(xPos, yPos);
		if(c.mouseX >= xPos - myWidth && c.mouseX <= xPos + myWidth && c.mouseY >= yPos - myHeight && c.mouseY <= yPos + myHeight)
			c.drawTooltip(c.mouseX + 11, c.mouseY - 20, toolTipText, true, info );
	}
	...
	public Sprite(String img, boolean hd) {
		try {			
			//imageName = img;
	...
	public Sprite(String img) {
		try {
			Image image;
			if(!img.endsWith(".gif")) {
				image = Toolkit.getDefaultToolkit().getImage(location + img + ".png");
			} else {
				image = Toolkit.getDefaultToolkit().getImage(location + img + "");
			}
			ImageIcon sprite = new ImageIcon(image);
			myWidth = sprite.getIconWidth();
			myHeight = sprite.getIconHeight();
	...
	public void method355(int ai[], int i, byte abyte0[], int j, int ai1[], int k,
