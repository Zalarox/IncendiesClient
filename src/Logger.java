...
public class Logger extends JFrame {
	...
	public static void addLog(String type, String text, Color c, Color c2) {
		...
		try { doc.insertString(doc.getLength(), ""+type+"",style); }
		catch (BadLocationException e){}
	...
	public Logger() throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				Client.stream.writeWordBigEndian(txtCommandsGoHere.getText().length() + 1);
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(javax.swing.JOptionPane.showConfirmDialog(null, 
						"Are you sure you would like to clear?", "Are you sure?", 
	...
	public void readLog() {
		/*Scanner s = null;
		try {
			s = new Scanner(new File(signlink.findcachedir()+ "ConsoleLogger.txt"));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		while(s.hasNextLine()) {
			if(s.nextLine().contains(".java:"))
				Logger.addLog("ERROR", s.nextLine(), Color.RED, Color.ORANGE);
			else {
				if(s.hasNextLine() && s.nextLine() != null && s.hasNext())
					Logger.addLog("INFO", s.nextLine(), Color.CYAN, Color.MAGENTA);
			}
		}*/
	}
}
